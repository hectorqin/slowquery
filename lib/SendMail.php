...
class SMTP {
    ...
    */
    public function getDebug(){
        if(!isset($this->_debugArray)) {
            $this->_debugArray = array();
        }
    ...
    */
    protected function sendCommand($command, $code) {
        $this->debug('Send command:' . $command . ',expected code:' . $code . '<br />');
        //发送命令给服务器
        try{
            if(socket_write($this->_socket, $command, strlen($command))){
                ...
                $this->debug('response:' . $data . '<br /><br />');
...
* @return   boolean
*/
function sendMail($address, $title, $content, $attachment = null){
    $mail = new SMTP();
    $mail->setServer(env('EMAIL_HOST'), env('EMAIL_USERNAME'), env('EMAIL_PASSWORD'), env('EMAIL_PORT'), true); //设置smtp服务器，到服务器的SSL连接
    $mail->setFrom(env('EMAIL_FROM'), env('EMAIL_FROMNAME')); //设置发件人
    foreach ((array)$address as $addr) {
        $mail->setReceiver($addr);  //设置收件人，多个收件人，调用多次
    }
    ...
    return $mail->sendMail() ? true : $mail->error();
}